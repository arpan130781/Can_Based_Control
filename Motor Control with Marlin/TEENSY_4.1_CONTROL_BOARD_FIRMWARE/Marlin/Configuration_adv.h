...
#define HOMING_BUMP_MM      { 6, 6,6, 6, 6 }       // (linear=mm, rotational=°) Backoff from endstops after first bump
...
// ================= CAN FEEDER SUPPORT =================
#define CAN_FEEDER_SUPPORT
// BUGFIX: CAN_FEEDER_MAX must match NUMBER_OF_FEEDERS in
// STM32F103_Slave/src/main.cpp (feederPins[] has 20 entries, indices
// 0-19). It was previously 32, allowing M755/M155 to silently accept
// feeder indices the slave doesn't have hardware for.
// BUGFIX: the old CAN_FEEDER_BASE_ID (0x200) has been removed. It
// collided with ACK_ID_BASE (0x200 + slave_id) and was never the ID
// the slave actually listens on (CMD_ID_BASE + SLAVE_ID = 0x101), so
// frames sent to it were always dropped by the slave as "ignored".
// M755 now reuses CAN_CMD_ID_BASE from can_motion_protocol.h instead.
#define CAN_FEEDER_MAX     20
// ======================================================

// ================= CUSTOM M155 FEEDER COMMAND =================
/**
 * CUSTOM_M155_COMMAND
 *
 * Enables a custom M155 handler that drives STM32F103 CAN feeder slaves:
 *
 *   M155 L<slave_id> D<feeder_index> S<speed>
 *
 *   L : Slave ID        — matches SLAVE_ID in the STM32F103 slave firmware.
 *                         CAN frame is sent to 0x100 + L.
 *                         Slave ACKs on 0x200 + L.
 *   D : Feeder index    — 0–19  (20 feeders per slave, feederPins[] array)
 *   S : Speed / PWM     — 0=stop, 1–254=PWM duty, 255=full ON (digital HIGH)
 *
 * CAN frame payload (4 bytes, matches slave processMessage() parser):
 *   [0] 0x10  CMD_START_MOTOR
 *   [1] D     feeder index
 *   [2] S     speed (low byte of uint16_t rpm)
 *   [3] 0x00  speed high byte (always 0 for 8-bit speed values)
 *
 * When this flag is set the stock AUTO_REPORT_TEMPERATURES M155
 * implementation is intentionally disabled to prevent a duplicate
 * symbol. If you need both, assign this handler a different code number.
 */
#define CUSTOM_M155_COMMAND

#if ENABLED(CUSTOM_M155_COMMAND)
  // Number of feeders per slave board (feederPins[] length in slave firmware)
  // BUGFIX: was 27, but STM32F103_Slave/src/main.cpp only defines 20
  // feeder pins (NUMBER_OF_FEEDERS = 20). Indices 20-26 were being
  // silently accepted here and then silently rejected by the slave.
  #define CUSTOM_M155_FEEDER_MAX   20
#endif
// =============================================================
...
#endif
#if defined(CAN_FEEDER_SUPPORT)
  #define HAS_CAN_FEEDER 1
#endif

// ================= CAN GENERIC AXIS MOTION SUPPORT =================
/**
 * CAN_AXIS_SUPPORT
 *
 * Enables forwarding of G0/G1 axis moves and G28 homing commands
 * to STM32F103 slave boards over CAN bus.
 *
 * Protocol (defined in custom/can_motion_protocol.h):
 *   CMD_AXIS_MOVE (0x20): G0/G1 move → slave steps motor
 *   CMD_AXIS_HOME (0x21): G28 home  → slave resets position to 0
 *
 * CAN_AXIS_SLAVE_ID: Which slave receives axis motion commands.
 *   Default 1 → CAN ID 0x101 (cmd) / 0x201 (ack)
 *
 * Axes Z (index 2), A/I (index 3), B/J (index 4) are handled by the
 * STM32F103 slave defined in STM32F103_Slave/src/main.cpp.
 */
#define CAN_AXIS_SUPPORT
#if ENABLED(CAN_AXIS_SUPPORT)
  #define CAN_AXIS_SLAVE_ID  1     // Slave that drives Z, A, B steppers
#endif
// ===================================================================
