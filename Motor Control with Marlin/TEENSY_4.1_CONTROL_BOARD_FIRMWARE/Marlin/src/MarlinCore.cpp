...
#include "HAL/shared/cpu_exception/exception_hook.h"
  ...
  #include "HAL/TEENSY40_41/canbus.h"
#endif
...
// =========================================================================
// ================ CLOSED-LOOP VACUUM CONTROL SYSTEM ======================
// =========================================================================

// --- HARDWARE CONFIGURATION (Teensy 4.1 T41U5XBB) ---
#define VAC_SENSOR_PIN   41                                                                                                                                                                                                                                                                                                                           // pin no 19    // WARNING: Pin 5 is Digital-Only on Teensy 4.1. Use A0-A17 for Analog!
#define VAC_SOLENOID_PIN 19    // Solenoid Output Pin

// --- CALIBRATION SETTINGS ---
// Teensy ADC is usually 10-bit (0-1023) or 12-bit (0-4095) in Marlin.
// Assuming 10-bit default for compatibility. 3.3V System.
// Adjust these values based on your sensor readings.
const int VAC_MIN_THRESHOLD = 800; // Lower Threshold (Turn ON)
const int VAC_MAX_THRESHOLD = 450; // Upper Threshold (Turn OFF)

// Control interval (ms)
const millis_t VAC_CHECK_INTERVAL = 100;

void vacuum_system_init() {
  // Initialize Solenoid
  pinMode(VAC_SOLENOID_PIN, OUTPUT);
  digitalWrite(VAC_SOLENOID_PIN, LOW); // Default OFF
  
  // Initialize Sensor Pin
  pinMode(VAC_SENSOR_PIN, INPUT);
}

void vacuum_system_update() {
  // Use static variable to track the last check time
  static millis_t next_vac_check_ms = 0;
  
  // Non-blocking timer check
  millis_t now = millis();
  if (ELAPSED(now, next_vac_check_ms)) {
    next_vac_check_ms = now + VAC_CHECK_INTERVAL;

    // Read vacuum level
    // If using a Digital Switch on Pin 5, use digitalRead() instead.
    int current_vac_level = analogRead(VAC_SENSOR_PIN); 
    // Serial.print("[VACUUM] Current Level = ");
    // Serial.println(current_vac_level);
    // --- HYSTERESIS LOGIC ---
    // Condition 1: Vacuum is too low (Needs Recharge) -> TURN ON
    if (current_vac_level  >= VAC_MIN_THRESHOLD) {
      if (digitalRead(VAC_SOLENOID_PIN) == LOW) {
        digitalWrite(VAC_SOLENOID_PIN, HIGH); 
      }
    }
    // Condition 2: Vacuum reached target Max (Full) -> TURN OFF
    else if (current_vac_level <=  VAC_MAX_THRESHOLD) {
      if (digitalRead(VAC_SOLENOID_PIN) == HIGH) {
        digitalWrite(VAC_SOLENOID_PIN, LOW);
      }
    }
  }
}
