...
#include "gcode.h"
#include "custom/M755.h"
#include "custom/M155.h"   // Custom M155: M155 L<led> D<dir> S<speed>
#include "custom/M756.h"   // CAN axis bridge: can_axis_move() / can_axis_home()
GcodeSuite gcode;
  ...
  */
  ...
  switch (parser.command_letter) {
    ...
    case 'M': switch (parser.codenum) {
      ...
      #if ALL(AUTO_REPORT_TEMPERATURES, HAS_TEMP_SENSOR) && DISABLED(CUSTOM_M155_COMMAND)
        case 155: M155(); break;                                  // M155: Set temperature auto-report interval
      #endif

      // ── Custom M155: M155 L<led_index 0-7> D<direction 0|1> S<speed 0-255> ──
      // Compiled only when CUSTOM_M155_COMMAND is defined; mutually exclusive
      // with the AUTO_REPORT_TEMPERATURES guard above so case 155 is never
      // emitted twice in the same translation unit.
      #if ENABLED(CUSTOM_M155_COMMAND)
        case 155: M155(); break;                                  // M155: Custom LED control
      #endif
      // ─────────────────────────────────────────────────────────────────────
