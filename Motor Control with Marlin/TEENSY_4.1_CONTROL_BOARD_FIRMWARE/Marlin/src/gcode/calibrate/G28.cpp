...
void GcodeSuite::G28() {
  ...
  // ── CAN generic axis homing ───────────────────────────────────────────
  // For every axis that was actually homed in this G28 call, send
  // CMD_AXIS_HOME (0x21) to the slave so it resets its position counter.
  // The slave does NOT move its motor — it only resets its tracking var.
  //
  // ADDING A NEW AXIS: nothing changes here. The do_axes[] array uses
  // NUM_AXIS_LIST which expands automatically with your Configuration.h.
  #if ENABLED(CAN_AXIS_SUPPORT)
    const bool do_axes[] = {
      NUM_AXIS_LIST(doX, doY, doZ, doI, doJ, doK, doU, doV, doW)
    };
    for (uint8_t i = 0; i < NUM_AXES; i++) {
      if (do_axes[i]) can_axis_home(i);
    }
  #endif
